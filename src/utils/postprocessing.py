...
def analyze_model_results(df_dict, stage_dict=None, stage_thresh=0.05):
    ...
    # Store proportions of wet days here
    wet_days_proportion = dict.fromkeys(df_dict.keys())
    ...
    for site, df in df_dict.items():
        ...
        if stage_dict and site in stage_dict:
            ...
            missing_ix = stage_series[stage_series.isna()].index

            if len(wet_ix):                             # skip if no wet days
                wet_days_proportion[site] = len(wet_ix) / (len(df) - len(missing_ix))
    ...
    return master_df, master_df_low, master_df_wet, wet_days_proportion
